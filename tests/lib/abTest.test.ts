...
describe('assignVariant', () => {
  ...
  it('returns stored assignment on repeated calls without recomputing', () => {
    ...
    expect(localStorage.getItem('ab_hero_copy')).toBe(first)
  })

  it('persists assignment to localStorage under ab_<testName>', () => {
    localStorage.setItem('anon_id', crypto.randomUUID())
    const variant = assignVariant('my_test', VARIANTS)
    expect(localStorage.getItem('ab_my_test')).toBe(variant)
  })

  it('distributes across variants: 100 distinct users do not all get the same variant', () => {
    const seen = new Set<string>()
    for (let i = 0; i < 100; i++) {
      localStorage.removeItem('ab_spread_test')
      localStorage.setItem('anon_id', `spread-user-${i}`)
  ...
  it('covers every variant: each option is assigned to at least one user across 300 seeds', () => {
    const seen = new Set<string>()
    for (let i = 0; i < 300; i++) {
      localStorage.removeItem('ab_coverage')
      localStorage.setItem('anon_id', `coverage-${i}`)
  ...
  it('distributes roughly evenly across a binary test (within ±20% of 50%)', () => {
    ...
    const N = 500
    for (let i = 0; i < N; i++) {
      localStorage.removeItem('ab_balance')
      localStorage.setItem('anon_id', `balance-user-${i}`)
  ...
  it('different test names are assigned independently for the same user', () => {
    localStorage.setItem('anon_id', 'independent-user')
    const a = assignVariant('test_alpha', TWO)
    const b = assignVariant('test_beta',  TWO)

    expect(localStorage.getItem('ab_test_alpha')).toBe(a)
    expect(localStorage.getItem('ab_test_beta')).toBe(b)
  ...
  it('ignores a stored value that is not in the current variants list', () => {
    localStorage.setItem('anon_id', 'some-user')
    localStorage.setItem('ab_hero_copy', 'deleted_variant')
...
describe('getStoredVariant', () => {
  ...
  it('reflects a manually written localStorage value', () => {
    localStorage.setItem('ab_manual', 'variant_a')
