...
describe('FeedbackPanel', () => {
  it('shows AnswerResult.Correct label when correct=true', () => {
    render(<FeedbackPanel feedback={feedback} correct={true} question={mockQuestion} />)
    expect(screen.getByText(AnswerResult.Correct)).toBeInTheDocument()
  })

  it('shows AnswerResult.Wrong label when correct=false', () => {
    render(<FeedbackPanel feedback={feedback} correct={false} question={mockQuestion} />)
  ...
  it('applies green styling when correct', () => {
    const { container } = render(<FeedbackPanel feedback={feedback} correct={true} question={mockQuestion} />)
    expect(container.firstChild).toHaveClass('bg-green-light')
  })

  it('applies red styling when wrong', () => {
    const { container } = render(<FeedbackPanel feedback={feedback} correct={false} question={mockQuestion} />)
