...
export default function Nav({ currentRoute, wrongCount, isPremium, onSignIn }: NavProps) {
  return (
          ...
          {navItems.map(item => (
            ...
            >
              ...
              {item.label === 'Review' && wrongCount > 0 && (
                <span className="absolute top-1.5 right-1 w-2 h-2 rounded-full bg-red" aria-label={`${wrongCount} questions to review`} />
              )}
            </Link>
          ))}
