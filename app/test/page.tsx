...
export default function TestPage() {
                    ...
                    <div className="max-w-2xl mx-auto px-5 py-10">
                        <div className="text-center mb-8">
                            ...
                            <p className="text-stone-600 text-base leading-relaxed max-w-md mx-auto">
                                {testSet.length} questions across all modules.
                                Feedback is withheld until the results screen.
                                Score ≥{TEST_PASS_PCT}% to earn the BikeReady
                                badge.
                            </p>
                        </div>
    ...
    async function handleAnswer(optionId: string, correct: boolean) {
        ...
        if (index + 1 >= testSet.length) {
            ...
            const scorePct = Math.round(
                (allAnswers.filter((a) => a.correct).length /
                    allAnswers.length) *
                    100,
            );
    ...
    const moduleBreakdown = modules
        .map((mod) => {
            const modAnswers = answers.filter(
                (a) => a.question.module === mod.id,
            );
    ...
    function handleShare() {
        ...
        if (navigator.share) {
            navigator
                .share({ text, url: "https://bikeready.app" })
                .catch(() => {});
        } else {
