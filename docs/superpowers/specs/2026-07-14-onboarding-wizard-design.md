...
Every screen shows: a 4-dot progress indicator, a plain **Skip** control (same on every step), and (steps 2–4) a **Back** control. Escape and backdrop click both skip. Focus stays trapped in the dialog (`useModalFocus`, already in place).
...
- **`tests/components/OnboardingOverlay.test.tsx` (rewrite):**
  ...
  - skipping on the final step fires `onboarding_skipped` with `{ step: 3 }` and calls `onSkip`.
